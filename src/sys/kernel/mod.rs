...
use procfs_core::{ProcError, ProcResult};
use std::cmp;
...
impl Version {
    ...
    pub async fn current(procfs: &AsyncProcfs) -> ProcResult<Self> {
        procfs.io().read_value("sys/kernel/osrelease").await
    }

    /// Cached version of the kernel version.
    pub(crate) async fn cached(procfs: &AsyncProcfs) -> ProcResult<Self> {
        ...
        if kernel == SENTINEL {
            kernel = Self::current(procfs).await?.to_u32();
...
impl Type {
    ...
    pub async fn current(procfs: &AsyncProcfs) -> ProcResult<Self> {
        procfs.io().read_value("sys/kernel/ostype").await
    }
}
...
impl BuildInfo {
    ...
    pub async fn current(procfs: &AsyncProcfs) -> ProcResult<Self> {
        procfs.io().read_value("sys/kernel/version").await
    }
    ...
    pub fn extra_date(&self) -> ProcResult<chrono::DateTime<chrono::Local>> {
        ...
        Err(ProcError::Other(
            "Failed to parse extra field to date".to_string(),
        ))
    }
}
...
pub async fn pid_max(procfs: &AsyncProcfs) -> ProcResult<i32> {
    procfs.io().read_value("sys/kernel/pid_max").await
}
...
impl SemaphoreLimits {
    pub async fn new(procfs: &AsyncProcfs) -> ProcResult<Self> {
        procfs.io().read_value("sys/kernel/sem").await
    }
...
pub async fn shmall(procfs: &AsyncProcfs) -> ProcResult<u64> {
    procfs.io().read_value("sys/kernel/shmall").await
}
...
pub async fn shmmax(procfs: &AsyncProcfs) -> ProcResult<u64> {
    procfs.io().read_value("sys/kernel/shmmax").await
}

/// Sets the limit on the maximum (System V IPC) shared memory segment size.
///
/// See also [shmmax](crate::sys::kernel::shmmax)
pub async fn set_shmmax(procfs: &AsyncProcfs, new_value: u64) -> ProcResult<()> {
    procfs
        .io()
        .write_value("sys/kernel/shmmax", new_value)
        .await
}
...
pub async fn shmmni(procfs: &AsyncProcfs) -> ProcResult<u64> {
    procfs.io().read_value("sys/kernel/shmmni").await
}
...
pub async fn sysrq(procfs: &AsyncProcfs) -> ProcResult<SysRq> {
    procfs.io().read_value("sys/kernel/sysrq").await
}

/// Set functions allowed to be invoked by the SysRq key
pub async fn set_sysrq(procfs: &AsyncProcfs, new: SysRq) -> ProcResult<()> {
    procfs
        .io()
        .write_value("sys/kernel/sysrq", new.to_number())
        .await
}
...
pub async fn threads_max(procfs: &AsyncProcfs) -> ProcResult<u32> {
    procfs.io().read_value("sys/kernel/threads-max").await
}
...
pub async fn set_threads_max(procfs: &AsyncProcfs, new_limit: u32) -> ProcResult<()> {
    if let Ok(kernel) = Version::cached(procfs).await
        && kernel.major >= 4
        && kernel.minor >= 1
        && !(THREADS_MIN..=THREADS_MAX).contains(&new_limit)
    {
    ...
    procfs
        .io()
        .write_value("sys/kernel/threads-max", new_limit)
        .await
}

pub struct AsyncProcfsSysKernel<'a> {
    inner: &'a AsyncProcfs,
}

impl<'a> AsyncProcfsSysKernel<'a> {
    pub fn new(procfs: &'a AsyncProcfs) -> Self {
        Self { inner: procfs }
    }

    pub async fn current_version(&self) -> ProcResult<Version> {
        Version::current(self.inner).await
    }

    pub async fn current_type(&self) -> ProcResult<Type> {
        Type::current(self.inner).await
    }

    pub async fn current_build_info(&self) -> ProcResult<BuildInfo> {
        BuildInfo::current(self.inner).await
    }

    pub async fn semaphore_limits(&self) -> ProcResult<SemaphoreLimits> {
        SemaphoreLimits::new(self.inner).await
    }

    pub async fn pid_max(&self) -> ProcResult<i32> {
        pid_max(self.inner).await
    }

    pub async fn shmmax(&self) -> ProcResult<u64> {
        shmmax(self.inner).await
    }

    pub async fn set_shmmax(&self, new_value: u64) -> ProcResult<()> {
        set_shmmax(self.inner, new_value).await
    }

    pub async fn shmmni(&self) -> ProcResult<u64> {
        shmmni(self.inner).await
    }

    pub async fn shmall(&self) -> ProcResult<u64> {
        shmall(self.inner).await
    }

    pub async fn sysrq(&self) -> ProcResult<SysRq> {
        sysrq(self.inner).await
    }

    pub async fn set_sysrq(&self, new: SysRq) -> ProcResult<()> {
        set_sysrq(self.inner, new).await
    }

    pub async fn set_threads_max(&self, new_limit: u32) -> ProcResult<()> {
        set_threads_max(self.inner, new_limit).await
    }

    pub async fn threads_max(&self) -> ProcResult<u32> {
        threads_max(self.inner).await
    }
}
